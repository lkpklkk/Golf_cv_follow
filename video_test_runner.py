...
def main():
    parser = argparse.ArgumentParser(description="Run offline golf CV tests on videos.")
    parser.add_argument(
        "input",
        nargs="?",
    ...
    parser.add_argument(
        "--generate-dataset",
        action="store_true",
        help="Generate fixed-length pose-sequence training samples from annotations.",
    )
    parser.add_argument(
        "--dataset-config",
        type=Path,
        default=DEFAULT_DATASET_CONFIG,
        help=f"Dataset generation config file. Default: {DEFAULT_DATASET_CONFIG}",
    )
    parser.add_argument(
        "--annotation-file",
        type=Path,
        ...
        help=(
            "Annotation file used by offline tracking for saved ReID reference "
            f"crops. Default: {DEFAULT_ANNOTATION_FILE}"
        ),
    )
    ...
    videos = _discover_videos(input_path)
    if not videos:
    ...
    for video in videos:
        ...
        realistic_run = RealisticVideoRun(video, embedder, reference_embeddings)
...
class OfflineVideoRun:
    ...
    def _recover_with_reid(self, frame, people):
        ...
        best_id = _select_best_reid(scores)
        if best_id is None:
            return
...
class RealisticVideoRun(OfflineVideoRun):
    ...
    def __init__(self, video_path: Path, embedder, reference_embeddings=None):
        super().__init__(
            video_path,
    ...
    def process(self):
        ...
        for sample_number, frame_index in enumerate(sample_indices):
            ...
            # Use the video timestamp so the action buffer perceives the same
            # temporal gaps that a live camera would produce at this jittered rate.
    ...
    def _draw(self, frame, people, sample_number, source_frame_index):
        ...
        if self.action_prediction is not None:
            ...
            # Place below the status bar (which occupies first ~74px)
...
def load_annotation_map(annotation_file: Path):
...
def load_reference_embeddings(annotation: dict | None, embedder, annotation_dir: Path):
...
def generate_pose_dataset(dataset_config: dict):
    ...
    videos = _discover_videos(raw_video_dir)
    if not videos:
...
def _normalize_dataset_config(dataset_config: dict):
    ...
    # Resolve a bare model filename from the yaml relative to the project root
    # so the weights are found regardless of the working directory.
...
def _reference_matched_pose_keypoints(result, frame, reference_embeddings, embedder):
    ...
    selected_index = _select_best_reid(scores)
...
def _discover_videos(path: Path):
    if path.is_file():
...
def _largest_person(people):
    return max(people, key=lambda person: _box_area(person["box"]), default=None)
